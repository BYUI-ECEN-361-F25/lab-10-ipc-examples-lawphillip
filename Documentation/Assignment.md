# ECEN-361 Lab-10: IPC-Examples
     Student Name:  __________________Phillip Law_________________
...
<mark>___By using the semaphore Acquire it waits for the semaphore to be given before it activates the task </mark>
<br>
<br><br>

2.)	How long is the time between the button interrupt coming in and it being enabled again? <br>
<mark>__30 milliseconds as the StartDebounce gets notified then waits 30 milliseconds before activating the interrupt again </mark>
><br>
> <br>
...
<mark>No D4 and D3 do not toggle with a single press the lights toggle one after the other  <br><br>

4.)	Now change one of the priorities of these two tasks, re-compile,  and re-run.
How has the behavior changed?
<mark> The light with the higher priority is the only one that toggles__<br><br>
...
><mark>The display switches between the the "--" and the numbers counting __<br><br><p>


>8.)	Is there a ‘priority’ associated with the Mutex?  If so, how can it be changed?
><br>  
><mark>_There is no priority to the mutex but the priority to the tasks using the MUtex can be changed_<br><br>
<p>
...
><mark>_I did not notice an effect__<br><br>
>
---
<!--------------------------------------------------------------------------------->
...
><mark>_The other option is osTimerOnce which only runs the timer a single time ___<br><br>
...
>
><mark>_The software timer would allow the debounce to be used multiple times while the delay blocks the task until the delay is over _<br><br>
